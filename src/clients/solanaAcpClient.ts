...
export class SolanaAcpClient extends BaseAcpClient<SolanaInstructionLike[]> {
   ...
   * send through the sponsored multi-signer path, so
   * fees and rents — including first-activation sub_expiry — are prefunded
   * and no wallet needs SOL.
   */
  async completeSubscriptionJob(
    chainId: number,
    params: CompleteParams & { providerSigner?: SolanaSigner; providerAddress?: Address }
  ): Promise<string> {
    const rpc = this.provider.getRpc(chainId);
    const signer = this.provider.getSigner();
    ...
    // Since F-118 the provider does NOT co-sign: activation no longer allocates,
    // so its signature authorizes nothing. It is still NAMED, because the hook
    // credits it the proposed_terms rent refund. `providerSigner` is therefore
    // optional and used only to recover a job whose sub_expiry was never
    // pre-created; callers with no access to the provider's key may omit it.
    const providerFromParams =
      params.providerAddress ?? params.providerSigner?.address;
    if (providerFromParams && providerFromParams !== job.data.provider) {
      throw new Error(
        `completeSubscriptionJob: provider ${providerFromParams} is not the ` +
          `job's provider ${job.data.provider}.`
      );
    }
    ...
    // ~0.0084 SOL rent is not gas-sponsorable and is never reclaimed, so it
    // would be charged to the acting wallet on every complete.
    ...
    const result = await this.provider.sendInstructions(chainId, instructions, {
      // Empty since F-118 — requiredExtraSigner is always null now. The
      // provider is named in the fan-out slice but does not sign.
      extraSigners: [],
  ...
  private async completeSubscriptionStandalone(
    ...
    const result = await this.provider.sendInstructions(
      ...
      standaloneIxs,
      {
        // Empty since F-118 — see completeSubscriptionJob.
        extraSigners: [],
        retryGuard: standaloneGuard,
        // On FIRST activation this tx creates the sub_expiry PDA, so the wallet
        // would really pay that rent — not just a fee — if it were not
        // sponsored. It stays sponsored because it touches an ACP program, and
        // the provider co-signature rides along: extraSigners now applies on
        // every send path, so it no longer forces self-pay.
      }
    );
    return Array.isArray(result) ? result[0]! : result;
  }
  ...
  private async fundViaRouter(
    ...
    if (maybeFriid.exists && maybeFriid.data.intentId !== 0n) {
      ...
      if (intent.data.amount > 0n && !coreApprovalCovers) {
        ...
        // regardless of mint — which is why this case needed ALT compression
        // first (R-H6, the same-mint over-budget case, got the bracket but not
        // the compression and failed alchemy_requestFeePayer at ~1224 bytes).
        //
        // Every router fund is compressed now, so there is no longer a flag to
        // set: the bracket is no longer the discriminator for whether the ALT
        // applies. Kept as a comment because it records WHY compression became
        // unconditional.
    ...
    // The ATA creates stay INLINE, and the ALT below is what makes room for
    // them.
    //
    // They used to be broadcast first, in their own transaction, so the fund tx
    // would fit under 1232 bytes. That worked on size but broke sponsorship: an
    // ATA-only batch contains no ACP program, so `isAcpAction` is false and the
    // send is not sponsored. It fell through to microgas, where the fee is
    // covered but ATA *rent* is still charged to the acting wallet, so it failed
    // at 0 SOL with System ResultWithNegativeLamports (custom program error:
    // 0x1).
    //
    // The size pressure that forced the split was self-inflicted: this path
    // built a v0 transaction and then only compressed it against the persistent
    // ALT when `bracketed`. Measured on devnet, an uncompressed router fund is
    // 1203/1232 bytes with ZERO loaded addresses, while 11 of its 27 static
    // accounts are in the ALT and none of them are signers — roughly 341 bytes
    // left on the table. Compressing unconditionally brings it to ~862 and the
    // creates fit with room to spare.
    const prepared = this.wrapMany(chainId, [
      cuLimitIx(),
      ...ataCreateIxs,
      ...hookPreIxs,
    ...
    if (persistentAlt) {
      // Compress EVERY router fund, not just the bracketed ones.
      //
      // The `bracketed` gate was why a plain same-mint fund shipped
      // uncompressed at 1203/1232 bytes and had to have its ATA creates split
      // into an unsponsored transaction. Bracketed funds (foreign mint or
      // same-mint over-budget) are simply the case where the shortage bit
      // first; the compression was always available to the others.
      //
      // Safe to apply unconditionally: the table is cluster-static and shared
      // with the complete path, and an ALT entry a transaction does not
      // reference costs nothing. The only accounts that cannot compress are
      // signers, and none of this list's entries are signers — they are
      // programs, PDAs and the treasury.
  ...
  private async submitViaRouter(
    ...
    if (escrowProposal !== null) {
      ...
      if (escrowProposal.amount > 0n && !coreApprovalCovers) {
        ...
        // A bracket (foreign OR over-budget budget-mint) adds instructions
        // that push the sponsored complete past 1232 bytes. That used to select
        // a split-and-compress treatment; every router complete is now
        // ALT-compressed and keeps its ATA creates inline, so the bracket is no
        // longer a discriminator and there is no flag to set. Kept as a comment
        // because it records why compression became unconditional.
      }
    ...
    const persistentAlt = MULTI_HOOK_COMPLETE_ALT_ADDRESSES[chainId];
    // ATA creates stay INLINE on every path, foreign mint included.
    //
    // The foreign-mint case used to broadcast them separately, on the reasoning
    // that the tx was "small and sponsored like any other ATA create". It was
    // not sponsored: an ATA-only batch contains no ACP program, so
    // `isAcpAction` in privySolanaProviderAdapter is false and the send falls
    // through to microgas, where the fee is covered but the ATA *rent* is still
    // charged to the acting wallet. Silent on a funded wallet, a hard failure at
    // 0 SOL. Same defect the router fund path had.
    //
    // Measured on devnet, a foreign-mint complete is 1055/1232 bytes with the
    // ALT already applied (17 loaded addresses), so ~177 bytes are free and two
    // createIdempotent instructions need ~60-90. It fits.
  ...
  private async preCreateHookRentPdas(
    ...
    try {
      ...
      // sub_expiry pre-creation is NOT optional since F-118: activation no
      // longer allocates, so a job whose sub_expiry was never created cannot be
      // completed by anyone — the evaluator's complete reverts with
      // SubExpiryNotPreCreated, and no signature fixes it. Swallowing that here
      // would defer a hard failure to complete-time, where the provider may no
      // longer be reachable. The other pre-creates remain best-effort: their
      // rents merely fall back to the provider wallet at CPI depth 4.
      if (args.subExpiryPackageId !== null) {
        throw new Error(
          `pre_create_sub_expiry failed for package ${args.subExpiryPackageId}: ` +
            `${(e as Error)?.message ?? e}. The subscription cannot be ` +
            `completed until this succeeds — retry, or have the provider call ` +
            `pre_create_sub_expiry directly.`
        );
      }
      console.warn(
