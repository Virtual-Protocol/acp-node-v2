...
// ---------------------------------------------------------------------------
// Solana buyer — same lifecycle as src/examples/basic/buyer.ts, but the agent
// is backed by a Solana client (`solanaProvider`) instead of an EVM one.
//
// Buyer lifecycle:
//
...
//
//   • Skip evaluation     — omit `evaluatorAddress` (defaults to the zero
//                           address / system program). The contract
//                           auto-completes the job on submit: the lifecycle
//                           skips `job.submitted` entirely and goes
//                           `budget.set` → `job.funded` → `job.completed`.
...
//
// Solana specifics:
//   • chainId 500 = devnet, 501 = mainnet-beta (see SOLANA_*_CHAIN_ID). These
//     are ACP's internal Solana chain ids, not EVM chain ids.
//   • Wallet addresses are base58 (no `0x` prefix) and case-sensitive, so the
//     address helpers below differ slightly from the EVM example.
//   • The Privy Solana wallet is a distinct wallet from any EVM Privy wallet —
//     it has its own address/id, hence the SOLANA_BUYER_* env vars.
//
// Backend: Solana runs against the ACP dev/testnet backend
// (ACP_TESTNET_SERVER_URL = https://api-dev.acp.virtuals.io), not production —
// the production backend (api.acp.virtuals.io) does not expose the Solana
// /wallets/solana/sign-message proxy and returns 404. The URL is wired into all
// three components that talk to ACP: the provider (RPC + Privy signing proxy),
// the AcpApiClient (jobs/agents), and the SseTransport (chat + event stream),
// since they default to production otherwise. The seller this buyer pairs with
// must use the same backend.
//
// We also pass `privyAppId: TESTNET_PRIVY_APP_ID` — the Privy app the dev
// backend is associated with — so the signed `privy-app-id` header, the
// backend, and the wallet all reference the same environment. Your SOLANA_BUYER_*
// wallet and its authorization key must be provisioned in that same Privy app,
// or the proxied Privy call fails ("authorization signature is incorrect" / 500
// from /wallets/solana/sign-message). If TESTNET_PRIVY_APP_ID doesn't match the
// live dev deployment, override it with SOLANA_PRIVY_APP_ID in your .env.
//
...
// Compare case-sensitively — base58 is case-sensitive, unlike EVM hex.
...
async function main(): Promise<void> {
  ...
  //     target a specific seller by wallet address (e.g. from
  //     `SOLANA_SELLER_WALLET_ADDRESS` in your .env). Returns null if the
  //     wallet isn't registered.
  //
  ...
  if (!offering) {
    log.error(`agent ${shortAddr(agent.solWalletAddress)} has no offerings`);
    await buyer.stop();
    return;
  }
  log.info(
  ...
  const requirementData = {
    description: "Test request from solana-basic/buyer.ts example",
  };
