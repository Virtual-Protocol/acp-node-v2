...
// ---------------------------------------------------------------------------
// Subscription seller lifecycle:
//
//   Same on-chain flow as basic/seller.ts (job.created → requirement message →
//   setBudget → job.funded → submit → job.completed), but `setBudget` becomes
//   `setBudgetWithSubscription` whenever the buyer's requirement message
//   carries a `packageId`. Two cases:
//
//     • Buyer sent a `packageId` and the subscription is **not yet active**:
//         budget = offering.priceValue + subscription.price
//         (the first job pays for the offering AND activates the package)
//
//     • Buyer sent a `packageId` and the subscription IS active:
//         budget = offering.priceValue
//         (subsequent jobs are covered by the still-active package)
//
//     • Buyer omitted `packageId`: fall back to a plain `setBudget`
//         (offering price only, no subscription).
//
...
const log = {
  info: (m: string) => console.log(`[seller-sub] ${m}`),
  job: (id: string | number, m: string) =>
    console.log(`[seller-sub] [job ${id}] ${m}`),
  chat: (session: JobSession, from: string, content: string) =>
    console.log(
      `[seller-sub] [job ${session.jobId}] ${counterpartyRole(
        session,
        from
      )} ${shortAddr(from)}: ${content}`
    ),
  send: (session: JobSession, content: string) =>
    console.log(`[seller-sub] [job ${session.jobId}] me: ${content}`),
  warn: (m: string) => console.warn(`[seller-sub] [warn] ${m}`),
  error: (m: string, e?: unknown) =>
    console.error(`[seller-sub] [error] ${m}`, e ?? ""),
};
...
async function main(): Promise<void> {
  ...
  try {
    ...
    for (const s of subscriptionsByPackageId.values()) {
      log.info(
        `  - "${s.name}" packageId=${s.packageId} (${s.price} USDC / ${
          s.duration / 86400
        }D)`
      );
    }
  ...
  seller.on("entry", async (session: JobSession, entry: JobRoomEntry) => {
    ...
    if (
      ...
      // Subscription branch: the buyer flagged a package on the requirement
      // message. The first job activates the package (offering price +
      // subscription fee); every subsequent job between this client/provider
      // pair while the package is still active is covered by the subscription
      // — we set a budget of 0 and the SubscriptionHook skips re-charging.
      console.log("entry.packageId", entry.packageId);
      ...
      if (entry.packageId !== undefined) {
        ...
        try {
          ...
          log.job(
            session.jobId,
            isActive
              ? `set budget to 0 USDC (subscription ${subscription.name} already active — covered)`
              : `set budget to ${totalPrice} USDC (offering ${offering.priceValue} + subscription ${subscription.name} ${subscription.price} — activates package)`
          );
        } catch (err) {
          log.error(
            `setBudgetWithSubscription failed on job ${session.jobId}`,
            err
          );
        }
        return;
      ...
      // No subscription package on this requirement — fall back to a plain
      // budget. This path is only reachable if the offering is configured
      // without a SubscriptionHook (e.g. the buyer used a non-subscription
      // path on a hybrid offering).
      try {
